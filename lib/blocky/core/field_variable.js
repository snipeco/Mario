...
Blockly.FieldVariable.dropdownChange = function(text) {
var me=this;
bootbox.prompt("Enter the Variable name?", function(result) { 
	if (result === null) {
		var c="var1"; 
		if (text == Blockly.Msg.RENAME_VARIABLE) { var oldVar = me.getText(); text =  c&&c.replace(/[\s\xa0]+/g," ").replace(/^ | $/g,""); if (text) { Blockly.Variables.renameVariable(oldVar, text); } return null; } else if (text == Blockly.Msg.NEW_VARIABLE) { text = c&&c.replace(/[\s\xa0]+/g," ").replace(/^ | $/g,""); if (text) { Blockly.Variables.renameVariable(text, text); return text; } return null; } 
	}
	else {
		var c=result; 
		if (text == Blockly.Msg.RENAME_VARIABLE) { var oldVar = me.getText(); text =  c&&c.replace(/[\s\xa0]+/g," ").replace(/^ | $/g,""); if (text) { Blockly.Variables.renameVariable(oldVar, text); } return null; }
		else if (text == Blockly.Msg.NEW_VARIABLE) {
			text = c&&c.replace(/[\s\xa0]+/g," ").replace(/^ | $/g,""); if (text) { Blockly.Variables.renameVariable(text, text); return text; } return null; } } 
